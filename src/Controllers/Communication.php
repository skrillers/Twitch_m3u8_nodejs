...
class Communication {
    ...
    public function sms() {
        ...
        if ($recipient == "student" || $recipient == "staff" || $recipient == "instructor" || $recipient == "everyone" || $recipient == "branches" || $recipient == "schools") {
            if ($recipient == "everyone") {
                ...
                $notification = 'SMS verzonden naar iedereen door <strong>' . $user->fname . ' ' . $user->lname . '</strong>.';
            } elseif ($recipient == "branches") {
                $recipients   = Database::table('branches')->where("school", $user->school)->get();
                $type         = "branch";
                $messageTable = "branchmessages";
                $notification = 'SMS verzonden naar alle vestigingen door <strong>' . $user->fname . ' ' . $user->lname . '</strong>.';
            } elseif ($recipient == "schools") {
                $recipients   = Database::table('schools')->get();
                $type         = "school";
                $messageTable = "schoolmessages";
                $notification = 'SMS verzonden naar alle scholen door <strong>' . $user->fname . ' ' . $user->lname . '</strong>.';
            } else {
                $notification = 'SMS naar iedereen verzonden ' . $recipient . 's by <strong>' . $user->fname . ' ' . $user->lname . '</strong>.';
        ...
        if (env("DEFAULT_SMS_GATEWAY") == "africastalking") {
            ...
            return response()->json(responder("success", "Oke!", "Bericht is verzonden. ", "reload()"));
            ...
            return response()->json(responder("success", "Oke!", "Je bericht is verzonden. ", "reload()"));
    ...
    public function email() {
        ...
        if ($recipient == "student" || $recipient == "staff" || $recipient == "instructor" || $recipient == "everyone" || $recipient == "branches" || $recipient == "schools") {
            if ($recipient == "everyone") {
                $recipients   = Database::table('users')->where("school", $user->school)->get();
                $notification = 'Email verzonden naar iedereen <strong>' . $user->fname . ' ' . $user->lname . '</strong>.';
            } elseif ($recipient == "branches") {
                $recipients   = Database::table('branches')->where("school", $user->school)->get();
                $type         = "branch";
                $messageTable = "branchmessages";
                $notification = 'Email verzonden naar alle filialen door <strong>' . $user->fname . ' ' . $user->lname . '</strong>.';
            } elseif ($recipient == "schools") {
                $recipients   = Database::table('schools')->get();
                $type         = "school";
                $messageTable = "schoolmessages";
                $notification = 'Email verzonden naar alle scholen door <strong>' . $user->fname . ' ' . $user->lname . '</strong>.';
            } else {
                $notification = 'Email verzonden naar iedereen ' . $recipient . 's by <strong>' . $user->fname . ' ' . $user->lname . '</strong>.';
                $recipients   = Database::table('users')->where("role", $recipient)->where("school", $user->school)->get();
            }
            Landa::notify($notification, $user->id, "message");
            foreach ($recipients as $account) {
    ...
    public function delete() {
        ...
        return response()->json(responder("success", "Bericht verwijderd", "Bericht succesvol verwijderd", "reload()"));
    }
