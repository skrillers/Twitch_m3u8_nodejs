...
class Database {
     ...
     * @return DB
     */
    public function count($column, $alias, $distinct = false) {
        if ($distinct) {
            return $this->get("COUNT(DISTINCT `{$column}`) AS `{$alias}`");
        }else{
    ...
    public function get() {
        ...
        return $this->getResults("SELECT {$this->getFields()} FROM {$this->tableName} WHERE {$this->getConditions()}{$this->order}{$this->limit}{$this->groupBy}");
    }
